    ...
    on_ground_(false)
{
    ...
    decceleration_ = entity_json_->get("decceleration").asFloat();
    gravity_ = entity_json_->get("gravity").asFloat();
    velocity_x_ = 0.0f;
    velocity_y_ = 0.0f;
}

void Player::update(int elapsed_time) {
    update_move(elapsed_time);
    update_jump(elapsed_time);
...
void Player::update_move(int elapsed_time) {
...
void Player::update_jump(int elapsed_time) {
    ...
    if(!jump_.active()) {
        velocity_y_ = std::min(velocity_y_ + gravity_ * elapsed_time, max_velocity_y_);
    }
